    ...
    'submit #lead_form' : function (event, template) {
      ...
      if(solved_questions_count < MAX_QUESTIONS){
        ...
        Honeybadger.notify(error, {
          message: 'Lead form submission issue',
          name: 'LeafForm',
          action: 'submit_lead_form',
          fingerprint: 'lead_not_submitted'
        });
      }
    },
  ...
  Template.questions.events({
    'change .question input:radio': function(event){
      ...
      $(event.target).parent().find(".css-label").addClass("selected-answer");
        ...
        Honeybadger.notify(error, {
          message: 'Error in moving to next question',
          name: 'Question',
          action: 'check_box_change',
          fingerprint: 'error_moving_to_next_question',
        });
      }
    },
  ...
  Meteor.startup(function() {
    ...
    bind_get_started = function() {
      ...
      update_iframe_height();
        ...
        Honeybadger.notify(error, {
          message: 'Error in starting assessment',
          name: 'Question',
          action: 'bind_get_started',
          fingerprint: 'error_getting_started',
        });
      }
    };
    ...
    bind_show_next_question = function() {
      ...
      update_progress_bar(next_question_number);
        ...
        Honeybadger.notify(error, {
          message: 'Error showing next question',
          name: 'Question',
          action: 'bind_show_next_question',
          fingerprint: 'error_displaying_next_question',
        });
      }
    };
    ...
    bind_show_previous_question = function() {
      ...
      update_progress_bar(prev_question_number);
        ...
        Honeybadger.notify(error, {
          message: 'Error in moving to previous question',
          name: 'Question',
          action: 'bind_show_previous_question',
          fingerprint: 'error_moving_to_previous_question',
        });
      }
    };
    ...
    bind_show_lead_form = function() {
      ...
      $(".lead-form-icon").removeClass("hidden");
        ...
        Honeybadger.notify(error, {
          message: 'Error showing lead form',
          name: 'LeadForm',
          action: 'bind_show_lead_form',
          fingerprint: 'error_showing_lead_form',
        });
      }
    }
  });
...
if(Meteor.isServer) {
  Meteor.startup(function() {
    ...
    BrowserPolicy.content.allowOriginForAll("*.pingdom.net/")
    BrowserPolicy.content.allowOriginForAll("*.marketo.net/")
    BrowserPolicy.content.allowOriginForAll("https://js.honeybadger.io/")
    BrowserPolicy.content.allowOriginForAll("https://www.googletagmanager.com/")
    BrowserPolicy.content.allowOriginForAll("*.burly.io/")
  });
}
